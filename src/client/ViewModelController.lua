...
-- ADS alignment offset: alpha from 0 (hipfire) to 1 (full ADS alignment).
-- Blends smoothly in RenderStepped to align iron sights with screen center while ADS.
-- Target 1 when adsState is Entering or Aiming, target 0 when Hip.
...
function ViewModelController:Start()
    ...
    RunService.RenderStepped:Connect(function(dt: number)
        ...
        -- ADS alignment alpha: blend toward 1 when ADS active, toward 0 when Hip.
        -- Target 1 when Entering or Aiming (needs alignment), 0 when Hip or Exiting (return to hipfire).
        local targetAlpha = 0
        if adsState == "Entering" or adsState == "Aiming" then
            targetAlpha = 1
        elseif adsState == "Exiting" then
            -- While exiting, smoothly blend back to 0 to avoid visual snap when adsOut plays.
            targetAlpha = 0
        end
        -- Lerp adsAlignmentAlpha toward target.
        ...
        -- ADS alignment offset: only applied when adsAlignmentAlpha > 0.
        ...
        if adsAlignmentAlpha > 0 then
            ...
            -- Build rotation offset (degrees → radians).
            local rotX = math.rad(Constants.VIEWMODEL_ADS_ALIGNMENT_ROTATION_X_DEGREES) * adsAlignmentAlpha
            local rotY = math.rad(Constants.VIEWMODEL_ADS_ALIGNMENT_ROTATION_Y_DEGREES) * adsAlignmentAlpha
            local rotZ = math.rad(Constants.VIEWMODEL_ADS_ALIGNMENT_ROTATION_Z_DEGREES) * adsAlignmentAlpha
            adsAlignmentCF = CFrame.new(offsetX, offsetY, offsetZ) * CFrame.Angles(rotX, rotY, rotZ)
        end
        ...
        -- Order: CAMERA_EXTRA_OFFSET (base hipfire) → adsAlignmentCF (ADS-only alignment)
        --        → viewRecoilCFrame (rotational recoil) → moveCF (movement sway)
        --        → BASE_OFFSET (model-specific) → recoilOffset (positional recoil) → adsIdleCF (fake idle).
        m:PivotTo(
            cam.CFrame
            * CAMERA_EXTRA_OFFSET
            * adsAlignmentCF
...
function ViewModelController:SetAiming(entering: boolean)
    ...
    if entering then
        ...
        -- ADS out: unfreeze adsIn, stop it, play adsOut, set state to Exiting.
        adsIdleTime = 0

        -- Stop and unfreeze adsIn if it's playing/frozen.

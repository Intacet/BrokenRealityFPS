...
**Regression introduced (2026-06-03 — commit 71386d9):** `adsAlignmentCF` (CFrame.new(-0.35, -0.15, 0.25)) was incorrectly removed.
...
**Update (2026-06-11 — isMoving() fix):** Root cause of the cadence skip found via MCP instrumentation. R6 locomotion applies impulses every ~3-4 physics frames; between impulses `AssemblyLinearVelocity` drops to 0.31–0.83 studs/s (below the 1.5 threshold), pausing the accumulator for ~25% of each stride and producing the audible gap. Fixed by replacing the velocity check with `Humanoid.MoveDirection.Magnitude > 0`. `MoveDirection` is driven by the input system, not the physics solver, and stays at 1.0 throughout any movement key hold. MCP-verified: 29 previously-skipped frames per 2 seconds are now correctly accumulated.

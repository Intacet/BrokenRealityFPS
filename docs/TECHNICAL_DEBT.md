...
**Remaining gaps (updated Stage 2I):**
- Crouch walk animation — supported if a `CrouchWalk` ID already exists in Constants for the active set; no ID has been added yet. Crouching while moving currently holds the EnterCrouch bottom pose at CROUCH_SPEED.
...
**Trigger:** Any playtesting session where T-pose during idle/jump or missing backward/diagonal animations is noticeable, where crouch bottom-pose hold looks wrong (visible snap on entry, wrong frame, or standing pose visible while crouched), where the `getRigDebugSummary` warn appears, or where animation speed or input feels wrong.
**Fix when:** A future movement stage adds idle/jump/fall/climb custom clips, backward/diagonal clips, CrouchWalk/CrouchIdle IDs, and server-owned equipment integration. Tune speed multipliers after first Studio playtest. Do not build full Scriptable camera system until camera refactor is scheduled.

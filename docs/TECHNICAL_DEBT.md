...
**Partially resolved (2026-05-15):** The server-side `local DEFAULT_WEAPON = "AR15"` in GunService was replaced with `Constants.DEFAULT_WEAPON` — the weapon name now has a single source of truth in `src/shared/Constants.lua`. `AmmoChanged` payload expanded from `(mag, reserve)` to `(weaponName, mag, reserve)`: GunService sends `Constants.DEFAULT_WEAPON` with every ammo update. HUD now displays the server-sent weapon name instead of the hardcoded "SCAR" string. GunController accepts `_weaponName` in the listener but ignores it for now (the client still uses its own `CURRENT_WEAPON` local for dry-fire and rate-limit lookups).
**Remaining risk:** `WeaponFired` and `ReloadRequest` still do not carry a weapon name — intentionally omitted because this task does not add multi-weapon support. GunController's `CURRENT_WEAPON = "AR15"` local still duplicates the server value (now both point to the same AR15 but through different mechanisms). A future weapon rename requires updating both Constants and GunController. The HUD label and ammo logic remain correct as long as there is only one weapon.
**MCP unavailable:** This change was not tested in Roblox Studio. The static code is correct but runtime behavior — HUD weapon label displaying the server-sent string, ammo counts remaining correct after the payload change — must be verified.
**Trigger for full resolution:** Multiple weapons exist as a player choice. Add `weaponName: string` to the `WeaponFired` payload (and to `ReloadRequest`), validate it exists in WeaponData on the server, and remove GunController's `CURRENT_WEAPON` local.
**Studio test required:** Join a session in ACTIVE phase; confirm HUD bottom-right shows "AR15" (not "SCAR" or blank). Fire a shot and reload; confirm ammo counts decrement and replenish correctly.

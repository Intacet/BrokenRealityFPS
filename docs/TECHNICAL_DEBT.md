...
## [DEBT-044] MovementController animation system — Unarmed directional animations — UPDATED 2026-05-20 (x16)
...
**Remaining gaps (updated Stage 2M):**
...
- First-person integration — when `FORCE_FIRST_PERSON = true` (ViewModelController), `CameraMinZoomDistance/CameraMaxZoomDistance` are overridden by Roblox's `LockFirstPerson` CameraMode. Stage 2K zoom limits and mouse-lock distance only apply while Classic CameraMode is active. No extra guard is needed now (they're additive), but verify behavior when first-person is enabled (DEBT-048).

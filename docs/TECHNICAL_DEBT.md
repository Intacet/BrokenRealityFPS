...
**Stable (2026-06-03 — ADS repair pass):** Repaired broken ADS animation state machine. Replaced fragile Stopped callback freeze with RenderStepped TimePosition monitoring. Added explicit ADSState type. No server changes, no weapon identity changes. DEBT-013 remains stable.
**Stable (2026-06-03 — ADS alignment tuning):** Added ADS-only viewmodel alignment offset to center iron sights on crosshair. Blends smoothly via adsAlignmentAlpha. No hipfire changes, no server changes, no weapon identity changes. DEBT-013 remains stable.
...
## [DEBT-040] ADS visual transition not implemented — REPAIRED 2026-06-03 (x2), NEEDS STUDIO VERIFICATION
...
**Current state:** ADS system exists and uses correct animations. Fake idle removed. Procedural movement suppressed during ADS. Track blending fixed. Epsilon-based early freeze removed — animation plays to completion. **NOT VERIFIED IN STUDIO** — MCP was unavailable for both repair passes.
**Remaining risk:** Stopped callback approach relies on Roblox preserving animation pose when TimePosition is set to Length and speed is set to 0 after the track has naturally stopped. If Roblox does not preserve the pose, the held ADS position may snap back to hipfire. If this occurs in Studio verification, document requirement for a real AKS74_FP_ADS_Idle animation asset instead of holding the enter animation final frame.
**Resolve when:** Studio verification confirms:
1. Right mouse while holstered → no errors, ADS does not activate
2. Equip AKS74 → right mouse → ADS enter animation plays **fully to its visible final pose** (no cutoff)
3. ADS enter completes before transitioning to Aiming state (animation finishes visibly)
4. ADS pose is held stable after enter finishes (no visual jumping/popping/pulling away)
5. Normal idle does not fight the held ADS pose
6. Run animation does not fight the held ADS pose
7. Procedural movement/sway/bob does not move the viewmodel while ADS is held
8. Fire while ADS → AKS_ADS_FIRE plays, returns to held ADS pose after
9. Right mouse again → ADS exit animation plays **fully to hipfire** (no cutoff)
10. Normal idle resumes after ADS exit finishes
11. Enter ADS → reload → ADS exits cleanly, reload plays
12. Enter ADS → holster → all ADS tracks stop, no pose remains stuck
...
- `CUSTOM_MOUSE_LOCK_REAPPLY_EVERY_FRAME = true` — **camera-jerk risk RESOLVED (2026-05-25, Stage 3E-fix-2):** The unconditional `MouseBehavior = LockCenter` write every Heartbeat caused Roblox's camera to re-centre its cursor-snap reference point 60 times per second. During sprint direction changes, the per-frame camera micro-reset compounded with the abrupt physics position change to produce a visible side-to-side camera jerk. Fix: conditional guard added — write only fires when `MouseBehavior ~= LockCenter`, eliminating 60 redundant per-second writes. MCP verified: 41/41 monitored frames had the value already correct; zero redundant writes under the new guard. CoreScript-theft protection unchanged. If a UI transition reads `MouseBehavior` to detect lock state (rather than calling `IsCustomMouseLocked()`), it should use `IsCustomMouseLocked()` instead — the conditional reapply no longer enforces LockCenter aggressively enough to mask API misuse.
...
- **Stage 2L new risk — sprint direction mismatch (resolved by Stage 3J):** Stage 2L simplified all sprint to RunForward. Stage 2R re-enabled ForwardLeft → RunForwardLeft / ForwardRight → RunForwardRight. Stage 3J reverts to RunForward always — Stage 3G body rotation (raw MoveDirection, lerp 0.18) provides all directional information. `RunForwardLeft` and `RunForwardRight` tracks are still loaded in `animationTracks` but are never selected at runtime. If a future stage wants to re-enable directional sprint clips, extend `getSprintAnimationName()` with a new gate constant without touching the Stage 3G body-rotation path.
...
- Directional sprint animations — Stage 3J: all sprint directions use RunForward. Stage 3G body rotation (MoveDirection, lerp 0.18) provides directional visual. RunForwardLeft/Right are loaded but never selected. If dedicated directional run clips are wanted later, extend `getSprintAnimationName()` with a new constant gate.
...
**Updated (2026-05-23 — Stage 2Q-D: crouch direct-blend / EnterCrouch disable):**
- EnterCrouch one-shot disabled by default (`CROUCH_USE_ENTER_TRANSITION_ANIMATION = false`). The original Unarmed and AR15 `EnterCrouch` assets had visually bad intermediate frames (forward-bend artifact). Disabling bypasses the artifact entirely without removing the asset. Re-enable by setting the constant to `true`.
- Direct-blend path: `playCrouchTransition(entering=true)` now directly crossfades into `CrouchIdle` (not-moving, 0.12s fade) or the appropriate `CrouchWalk*` direction (moving, 0.10s fade). `crouchTransitionPlaying` is explicitly cleared so Heartbeat immediately takes over animation maintenance.
- `wasMovingWhileCrouching = true` is set in the direct-blend path when moving, so the first Heartbeat skips the `CrouchWalkStart` check and goes straight to directional selection.
- `CrouchWalkStart` one-shot gated behind `CROUCH_USE_CROUCH_WALK_START_ANIMATION = false` by default. The forward-lunge artifact on the first crouched step is eliminated.
- Four new constants: `CROUCH_USE_ENTER_TRANSITION_ANIMATION`, `CROUCH_DIRECT_BLEND_FADE_TIME`, `CROUCH_DIRECT_BLEND_MOVING_FADE_TIME`, `CROUCH_USE_CROUCH_WALK_START_ANIMATION`.
- No HipHeight, CameraOffset, FOV, camera.CFrame, or server-side changes. No new animation IDs. No new remotes.
- MCP unavailable — Studio verification was not performed. Needs manual playtest: (1) C press while standing → crossfades directly to CrouchIdle without visible EnterCrouch bend; (2) C press while walking → crossfades directly to CrouchWalk* without CrouchWalkStart lunge; (3) ExitCrouch path unchanged (exits cleanly from CrouchIdle/CrouchWalk); (4) no animation blending artifacts; (5) WalkSpeed, HipHeight, and CameraOffset are unaffected.

- **Stage 2Q-D new risk — EnterCrouch asset still loads but is visually unused:** `loadMovementAnimations()` still loads the `EnterCrouch` track for both sets. This wastes a small amount of memory and one `LoadAnimation` call per respawn. When the asset is replaced with a good one, flipping `CROUCH_USE_ENTER_TRANSITION_ANIMATION = true` re-enables it immediately. If the asset is permanently discarded, remove the ID from `Constants.MOVEMENT_ANIMATION_IDS` and the `Looped=false` pattern check in `loadMovementAnimations`.
- **Stage 2Q-D new risk — direct-blend fade time may need tuning:** `CROUCH_DIRECT_BLEND_FADE_TIME = 0.12s` (not moving) and `CROUCH_DIRECT_BLEND_MOVING_FADE_TIME = 0.10s` (moving) were set without live Studio verification. If the crossfade looks abrupt or too slow, tune these constants. They are the primary visual feel knob for this path.
- **Stage 2Q-D new risk — directional CrouchWalk* selection when moving:** The direct-blend moving path attempts to pick the correct directional CrouchWalk track. If `customMouseLocked` is OFF or the directional key is absent, it falls back to `CrouchWalkForward` then `CrouchWalk` alias. If neither alias exists (e.g. AR15 set), it falls back to `holdCrouchBottomPose()`. All fallback branches are guarded and logged, but the AR15-set fallback was not live-verified.
- **Stage 2Q-D new risk — CrouchWalkStart flag cleared on stop/respawn, not reset:** `wasMovingWhileCrouching` is reset to `false` on movement stop and respawn. In the direct-blend path it is set to `true` when crouching while moving. If the flag is not correctly reset in all paths (phase exit, `loadMovementAnimations`, `destroy`), subsequent crouches may skip or re-trigger incorrectly. Verify in Studio that stand → crouch → uncrouch → crouch again always crossfades correctly without stale flag state.
- **Stage 3C fix risk — crouch bug (updated 2026-05-23):** Stage 2Q-D addresses the visual artifact (bad EnterCrouch frames / CrouchWalkStart lunge). Any remaining behavioral crouch bug (rapid-tap, sprint-to-crouch edge cases) still requires manual Studio playtest. The above manual test steps for Stage 2Q-D cover the primary paths.

**Updated (2026-05-23 — Stage 2S: Zero-gap crouch-exit transitions):**
- Crouch-to-locomotion pose gap fixed. Root cause: after `playCrouchTransition(false)` hard-stopped crouch tracks with `Stop(0)`, the ExitCrouch animation played while Heartbeat was gated (`crouchTransitionPlaying = true`). When ExitCrouch finished, Heartbeat resumed on the next tick — leaving ≥1 frame where all track weights were zero, producing the default Roblox neutral pose flash.
- Fix: `crouchEndConn` now uses a zero-gap path when `CROUCH_ZERO_GAP_TRANSITIONS_ENABLED = true` (default):
  - **Moving** (default `CROUCH_USE_EXIT_TRANSITION_WHILE_MOVING = false`): ExitCrouch is skipped. `resumeStandingLocomotionAfterCrouch(0.10s)` fades all crouch tracks out and the standing animation in **with the same fadeTime** — combined weight never hits zero, so the default pose cannot appear.
  - **Not moving**: ExitCrouch still plays; when it finishes, `resumeStandingLocomotionAfterCrouch` is called immediately in the Stopped callback (`CROUCH_EXIT_RESUME_LOCOMOTION_IMMEDIATELY = true`) — Idle starts inside the callback, not on the next Heartbeat.
  - No ExitCrouch track: falls directly to `resumeStandingLocomotionAfterCrouch`.
  - Legacy path preserved when `CROUCH_ZERO_GAP_TRANSITIONS_ENABLED = false`.
- Two new helpers: `getDesiredStandingLocomotionKey()` (pure read, mirrors `updateMovementAnimation` standing/sprint selection), `resumeStandingLocomotionAfterCrouch(fadeTime)` (crossfades with matched Stop/Play fadeTime).
- MCP Studio verified 2026-05-23: module loaded clean, all 5 new constants live, all 13 source patterns present, no Output errors in play mode. C key cannot reach `InputBegan` via MCP — full end-to-end visual crouch test requires manual Studio playtest.

- **Stage 2S new risk — `getDesiredStandingLocomotionKey` duplicates selection logic:** The helper mirrors `updateMovementAnimation`'s standing/sprint walk-direction selection verbatim. Any future change to that selection (new directions, new fallback priorities, new animation names) must be applied to **both** `updateMovementAnimation` and `getDesiredStandingLocomotionKey`. If they diverge, the immediate post-crouch animation will differ from what Heartbeat would select, causing a visible blip on the first frame. Consider a shared helper in a future refactor stage.
- **Stage 2S new risk — crossfade fade times may need tuning:** `CROUCH_EXIT_DIRECT_BLEND_FADE_TIME = 0.10s` and `CROUCH_EXIT_IDLE_BLEND_FADE_TIME = 0.12s` were set without live visual confirmation. If the transition feels too abrupt (pop) or too slow (floaty), tune these two constants. They are the primary feel knob for this path.
- **Stage 2S new risk — ExitCrouch Stopped callback uses inline `crouchTransitionConn`:** The not-moving path assigns a new Stopped callback to `crouchTransitionConn` after `clearCrouchTransitionConnection()` is called at the top of `crouchEndConn`. If `crouch_EndConn` fires again before ExitCrouch finishes (e.g. rapid C-tap), the second `clearCrouchTransitionConnection()` disconnects the first callback cleanly — the pattern is correct. Verify in Studio that rapid C-tap (press + release within 300ms) does not leave a stale callback or stuck state.
- **Stage 2S new risk — sprint held at crouch release:** `getDesiredStandingLocomotionKey()` reads `movementState.isSprinting` at the moment of `crouchEndConn`. If LeftShift is held when C is released, `isSprinting` may already be `true` (set by the Heartbeat sprint-input check) — the function returns a RunForward/sprint key rather than WalkForward. This is the correct desired behavior (crouch release while sprinting → resume sprint animation). Verify in Studio that C-release with LeftShift held blends directly into RunForward without a gap.
- **Stage 2S risk — C key not verified via MCP:** MCP `user_keyboard_input` tool does not reach `InputBegan` in Studio play mode. All structural and load-time checks passed; manual Studio playtest required to confirm: (1) crouch→walk blends without gap; (2) crouch→sprint blends without gap; (3) crouch→idle ExitCrouch plays then Idle starts immediately; (4) no WalkSpeed or HipHeight changes; (5) rapid C-tap does not corrupt state.

**Updated (2026-05-23 — Stage 3D: Sprint directional body-facing):**
- When custom mouse lock is active and the player sprints, the character body now rotates toward the camera-relative movement input direction (`Humanoid.MoveDirection` flat XZ, already camera-relative in Roblox) instead of always facing camera yaw. Walking still uses camera-yaw facing (Stage 2E unchanged).
- Two new helpers: `getCameraRelativeMoveDirection()` (reads `movementState.moveVector`, returns nil below threshold), `faceCharacterTowardsDirection(dir)` (CFrame.lookAt yaw-only, sets AutoRotate=false, optional LERP smoothing path).
- `applyCharacterFacing()` sprint branch: gates `SPRINT_DIRECTIONAL_BODY_FACING_ENABLED`, `isSprinting`, `not isTacticalSprinting`, `not isCrouching`. Calls move-dir facing; falls back to camera-yaw if below magnitude threshold.
- `getSprintAnimationName()` extended: `BackwardLeft` → RunForwardLeft if loaded else RunForward; `BackwardRight` → RunForwardRight if loaded else RunForward. Body rotation toward movement direction means RunForward plays in the correct world-space direction for all non-diagonal directions.
- New module-level state: `lastSprintFacingMode` (cleared on respawn/destroy).
- Six new constants: `SPRINT_FACE_MOVEMENT_DIRECTION_WHILE_MOUSE_LOCKED`, `SPRINT_DIRECTIONAL_BODY_FACING_ENABLED`, `SPRINT_DIRECTIONAL_BODY_FACING_MIN_MOVE_MAGNITUDE`, `SPRINT_DIRECTIONAL_BODY_FACING_DEBUG`, `SPRINT_DIRECTIONAL_BODY_FACING_SMOOTHING_ENABLED`, `SPRINT_DIRECTIONAL_BODY_FACING_LERP_ALPHA`.
- MCP unavailable — Studio verification was not performed. Needs manual playtest.

- **Stage 3D new risk — `getDesiredStandingLocomotionKey()` not updated for BackwardLeft/BackwardRight sprint:** The Stage 2S helper mirrors `updateMovementAnimation` standing/sprint selection. `getSprintAnimationName()` was extended for BackwardLeft/BackwardRight in Stage 3D; `getDesiredStandingLocomotionKey()` calls `getSprintAnimationName()` internally, so it inherits the new behavior automatically. No separate change needed, but verify during crouch-release-while-sprint-diagonal test.
- **Stage 3D new risk — body rotation may need smoothing after live play:** `faceCharacterTowardsDirection()` applies immediate rotation by default (`SPRINT_DIRECTIONAL_BODY_FACING_SMOOTHING_ENABLED = false`). If rapid direction changes during sprint look jittery (e.g. diagonal sprint with slight input noise), enable smoothing and tune `SPRINT_DIRECTIONAL_BODY_FACING_LERP_ALPHA` (try 0.2–0.4 per Heartbeat). Do not adjust without live visual confirmation in Studio.
- **Stage 3D new risk — run-left/run-right/run-backward dedicated animations still missing:** Left/Right/Backward sprint directions play RunForward while the body rotates. If a dedicated run-backward clip is added later, extend `getSprintAnimationName()` to prefer it. BackwardLeft/BackwardRight reuse RunForwardLeft/RunForwardRight as an approximation.
- **Stage 3D new risk — AutoRotate not restored when transitioning from sprint to non-sprint within mouse lock:** `faceCharacterTowardsDirection()` sets `AutoRotate = false` and returns. When the player stops sprinting while mouse lock is still active, `applyCharacterFacing()` takes over (camera-yaw path) and also keeps `AutoRotate = false`. AutoRotate is only restored when mouse lock is disabled (via `restoreCharacterAutoRotate()`) or phase exits. This is correct and intentional — but verify in Studio that `AutoRotate` is never stuck false after lock is disabled.
- **Stage 3D new risk — MCP Studio verification not performed:** Studio instance disconnected before constants could be verified live. Manual Studio playtest required to confirm: (1) sprint left → character faces left, RunForward plays; (2) sprint right → character faces right; (3) sprint backward → character faces backward; (4) sprint forward-left → RunForwardLeft or RunForward; (5) walking still uses camera-yaw facing; (6) AutoRotate restores on mouse-lock disable; (7) no camera.CFrame writes; (8) no HipHeight/CameraOffset/FOV changes.

**Updated (2026-05-23 — RunForwardTest toggle):**
- `Unarmed.RunForwardTest = rbxassetid://118179559114284` added to `Constants.MOVEMENT_ANIMATION_IDS.R6.Unarmed`. Loaded conditionally alongside `Unarmed_RunForward` on each character spawn.
- `MOVEMENT_RUN_FORWARD_USE_TEST_ANIMATION = false` added to Constants. When `true` and Unarmed, `getRunForwardSuffix()` returns `"RunForwardTest"` instead of `"RunForward"`. All call sites that previously returned `"RunForward"` (including the no-mouse-lock path, all directional fallbacks in `getSprintAnimationName()`, and both tactical-sprint fallbacks in `updateMovementAnimation` and `getDesiredStandingLocomotionKey`) now route through `getRunForwardSuffix()`. AR15 `RunForward` is unaffected.
- MCP unavailable — Studio verification was not performed. Needs manual playtest.

- **RunForwardTest new risk — toggle does not apply to AR15 set:** `getRunForwardSuffix()` returns `"RunForward"` unconditionally when `animSetName ~= Constants.MOVEMENT_ANIMATION_SET_UNARMED`. If an AR15 test animation is ever needed, a second toggle and ID pair must be added.
- **RunForwardTest new risk — unused track when toggle is false:** `Unarmed_RunForwardTest` loads on every character spawn regardless of toggle state (same pattern as `WalkForwardAlt`, `CrouchIdleAlt`, `TacticalSprintForward2`). This is one extra `LoadAnimation` call and one extra AnimationTrack slot per character. If the test is permanently discarded, remove the ID from Constants and the conditional-load block in `loadMovementAnimations()`.
- **RunForwardTest new risk — toggle must be flipped in Constants.lua, not at runtime:** There is no runtime API to switch between `RunForward` and `RunForwardTest`. Changing the constant requires a file edit and `rojo serve` sync. This is intentional (the toggle is a developer convenience, not a player-facing option), but it means the change cannot be hot-switched during a live play session without restarting Studio.
- **RunForwardTest MCP unavailable:** Studio verification was not performed. Manual test steps: (1) set `MOVEMENT_RUN_FORWARD_USE_TEST_ANIMATION = false` → sprint forward → confirm existing `RunForward` clip plays; (2) set to `true` → sprint forward → confirm test clip plays instead; (3) set back to `false` → confirm original clip restored; (4) AR15 set: confirm AR15 RunForward plays in both toggle states; (5) confirm no Output errors on character spawn; (6) confirm `WalkSpeed`, `HipHeight`, `CameraOffset`, `FOV` unchanged.

**Updated (2026-05-23 — Stage 3E: Natural AutoRotate sprint rotation):**
- **Stage 3D discrete-snap risk RESOLVED by Stage 3E:** The Stage 3D `faceCharacterTowardsDirection()` CFrame path has been replaced inside `applyCharacterFacing()`. During sprint + custom mouse lock, `Humanoid.AutoRotate = true` is now set and the function returns early — Roblox physics rotates the character naturally toward movement direction, eliminating the discrete 45°/90° snap at `directionName` boundaries. Walking, idle, crouching, tactical sprint, sprint-stop, and landing continue on the camera-yaw CFrame path with `AutoRotate = false`.
- **Stage 3D "AutoRotate not restored on sprint→walk transition" risk RESOLVED:** The Stage 3E exit block (`if lastNaturalSprintAutoRotateActive`) explicitly sets `hum.AutoRotate = false` on the first non-sprint Heartbeat after leaving sprint mode. AutoRotate is thus correctly re-enforced before the camera-yaw CFrame write, with no window where AutoRotate is stuck `true` during walking.
- **New helper `shouldUseNaturalSprintAutoRotate()`:** Gates on both `SPRINT_USE_NATURAL_AUTOROTATE_WHILE_MOUSE_LOCKED` and `SPRINT_DISABLE_MANUAL_BODY_FACING_WHILE_MOUSE_LOCKED` (independent kill switches), plus `customMouseLocked`, `isSprinting`, `not isTacticalSprinting`, `not isCrouching`, `not isSprintStopPlaying`, `not isLandingMovementLocked`.
- **New state variable `lastNaturalSprintAutoRotateActive`:** Tracks whether Stage 3E sprint mode is currently active. Cleared on respawn, mouse-lock disable (`applyCustomMouseLock()` off branch), and `destroy()`. Prevents per-frame spam logging and ensures the exit block runs exactly once per sprint→non-sprint transition.
- **Three new constants:** `SPRINT_USE_NATURAL_AUTOROTATE_WHILE_MOUSE_LOCKED = true`, `SPRINT_DISABLE_MANUAL_BODY_FACING_WHILE_MOUSE_LOCKED = true`, `SPRINT_NATURAL_AUTOROTATE_DEBUG = true`.
- `getCameraRelativeMoveDirection()` and `faceCharacterTowardsDirection()` are **retained** in the file (not deleted). Stage 3D constants (`SPRINT_DIRECTIONAL_BODY_FACING_ENABLED`, `SPRINT_FACE_MOVEMENT_DIRECTION_WHILE_MOUSE_LOCKED`, etc.) are also retained. Both are now dead code paths when Stage 3E flags are true.
- MCP unavailable — Studio verification was not performed. Needs manual playtest.

- **Stage 3E new risk — dead code from Stage 3D retained:** `getCameraRelativeMoveDirection()`, `faceCharacterTowardsDirection()`, `lastSprintFacingMode`, and all six Stage 3D constants remain in the files but are not called when Stage 3E flags are both true. If Stage 3E is confirmed good in Studio, these can be deleted in a future cleanup pass. Do not delete until Stage 3E is verified — they serve as the rollback path (set both Stage 3E constants to false and the Stage 3D code becomes reachable again... except the Stage 3D block itself was replaced. The rollback is: set `SPRINT_USE_NATURAL_AUTOROTATE_WHILE_MOUSE_LOCKED = false` → `applyCharacterFacing()` camera-yaw path runs for all sprint states, no faceCharacterTowardsDirection called — character always faces camera yaw while sprinting).
- **Stage 3E new risk — AutoRotate = true during sprint may conflict with future tactical-sprint camera effects:** Tactical sprint is correctly excluded from Stage 3E (`isTacticalSprinting` guard). Normal sprint with `AutoRotate = true` relies on `Humanoid.MoveDirection` being set by the engine from the player's input. Confirm in Studio that the character facing direction feels natural and predictable with every sprint direction key combination.
- **Stage 3E new risk — `isSprintStopPlaying` and `isLandingMovementLocked` gate correctness:** Both are included as guards to prevent AutoRotate from being set to true during sprint-stop animation or landing. If either flag has stale state (e.g. sprint-stop flag not cleared on respawn), Stage 3E may incorrectly stay on the camera-yaw path during legitimate sprint. Verify both flags are cleared on respawn and phase transitions.
- **Stage 3E MCP unavailable:** Studio verification was not performed. Manual test steps: (1) enable mouse lock (LeftAlt) → sprint forward → confirm character body rotates naturally toward movement; (2) sprint left → character faces left smoothly, no discrete snap; (3) sprint right → no snap; (4) sprint backward → no snap; (5) release sprint (walk while mouse-locked) → character immediately snaps to camera yaw (AutoRotate restored false); (6) tactical sprint → character still faces camera yaw (not Stage 3E); (7) disable mouse lock while sprinting → AutoRotate restored to original; (8) respawn while sprinting with mouse lock on → mouse lock released, state cleared; (9) confirm no camera.CFrame writes, no HipHeight/CameraOffset/FOV changes; (10) Output shows Stage 3E entry and exit logs exactly once per transition.
- **Stage 3E camera-jerk RESOLVED (2026-05-25):** `Humanoid.AutoRotate = true` caused visible left-right camera jerk when changing sprint direction in shift lock. Root cause: in custom mouse lock the camera is pinned to `HumanoidRootPart`'s yaw — when the Roblox physics engine rotated HRP toward `MoveDirection`, the camera rotated with it. Rapid direction changes (A↔D, diagonal flick) produced visible snapping. **Fix:** `SPRINT_USE_NATURAL_AUTOROTATE_WHILE_MOUSE_LOCKED = false`. `shouldUseNaturalSprintAutoRotate()` now returns false immediately; sprint falls through to the camera-yaw CFrame path (same as walking in mouse lock). Character always faces camera yaw during sprint. Directional sprint animations (`RunForwardLeft`/`RunForwardRight` via `getSprintAnimationName`) communicate movement direction visually without any body rotation. Stage 3E infrastructure retained as dead code for rollback.

**Updated (2026-05-25 — Stage 3F: Zero-gap landing exit):**
- Root cause: the `landingConn` Stopped callback in `playLandingAnimation()` called `clearLandingConnection()` (setting `isLandingPlaying = false`, releasing the Heartbeat gate) but did not immediately start the next locomotion animation. The next `updateMovementAnimation()` call on the following Heartbeat tick (~16ms later) selected and played the correct animation, leaving at least one frame where all track weights were zero — producing the visible T-pose / blank-pose flash on landing.
- Fix: at the end of the Stopped callback, call `playMovementAnimation(getDesiredStandingLocomotionKey())` when `LANDING_EXIT_RESUME_LOCOMOTION_IMMEDIATELY = true` and `not isLandingPlaying`. This mirrors the Stage 2S crouch-exit approach (`resumeStandingLocomotionAfterCrouch` → `getDesiredStandingLocomotionKey`).
- `getDesiredStandingLocomotionKey()` moved from the Stage 2S section (~line 2759) to just before `playLandingAnimation()` (~line 2662) so the landing Stopped callback can call it without a forward-reference (Luau `--!strict` scoping rule). All call-site dependencies (`getAnimationSetName`, `isMouseLockedForStrafeAnimations`, `getRunForwardSuffix`, `getSprintAnimationName`) were already defined before the new position. `resumeStandingLocomotionAfterCrouch` (Stage 2S) is still defined after `getDesiredStandingLocomotionKey` — satisfied.
- Stage 2S section header updated: duplicate `getDesiredStandingLocomotionKey` definition removed; comment updated to reference Stage 3F placement.
- **One new constant:** `LANDING_EXIT_RESUME_LOCOMOTION_IMMEDIATELY = true`.
- **MCP Studio verified 2026-05-25:** 25-stud drop — `LandingHeavy` (w=0.61) and `Idle` (w=0.78) overlapping in the same Heartbeat tick; no zero-weight gap frame. Trace: `t=2.70 [LandingHeavy w=0.61] [Idle w=0.78]` → `t=2.88 [LandingHeavy w=0.00] [Idle w=1.00]` → `t=2.90 [Idle w=1.00]`.

- **Stage 3F new risk — `getDesiredStandingLocomotionKey` duplication with Stage 2S still applies:** Stage 2S noted that the helper duplicates `updateMovementAnimation`'s selection logic. Stage 3F adds a second consumer (the landing Stopped callback). Any future change to standing animation selection must now be applied to both `getDesiredStandingLocomotionKey` and `updateMovementAnimation`. The risk is unchanged; the helper now has two call sites.
- **Stage 3F new risk — second landing before callback fires:** If a new landing starts before the first `landingConn` Stopped callback fires (e.g. double-bounce on geometry), `isLandingPlaying` will be `true` again when the callback runs. The guard `if not isLandingPlaying` prevents the old callback from overriding the new landing animation. Verify in Studio that rapid consecutive landings do not produce a crossfade interrupt.
- **Stage 3F new risk — crossfade on landing while moving vs. idle:** `getDesiredStandingLocomotionKey()` reads `movementState` at callback time. If the player releases movement keys during the landing animation, the helper correctly returns an Idle key. If input is still held, it returns the walk/run key. Both cases are desirable. Verify in Studio that landing while actively moving blends into the correct walk/run animation without a visible snap.

**Updated (2026-05-25 — Stage 3J: Disable RunForwardLeft/Right — RunForward plays for all sprint directions):**
- `getSprintAnimationName()` simplified: all directional branching removed. Previously resolved `ForwardLeft → RunForwardLeft`, `ForwardRight → RunForwardRight` (Stage 2R), and `BackwardLeft/Right → RunForwardLeft/Right` (Stage 3D). Now always returns `getRunForwardSuffix(animSetName)` for all directions. The `directionName` parameter is still accepted (API preserved) but not used.
- Stage 3G block in `applyCharacterFacing()` simplified: ForwardLeft/Right fixed-angle rotation path (Stage 3I) removed. All sprint directions now use the same `getCameraRelativeMoveDirection()` → `faceCharacterTowardsDirection()` path. The Stage 3I 2D rotation formula (`c/s math.rad`, `SPRINT_DIAGONAL_BODY_ROTATION_DEGREES`) is gone.
- `SPRINT_DIAGONAL_BODY_ROTATION_DEGREES` is now dead code — the constant is retained in `Constants.lua` for reference (value 20) but is never read at runtime.
- **Stage 3I risk SUPERSEDED:** "diagonal body rotation tuning" and "sign correctness for left/right rotation" risks from Stage 3I are obsolete. `SPRINT_DIAGONAL_BODY_ROTATION_DEGREES` is unused; the sign formula was removed. Stage 3I was a necessary intermediate step toward Stage 3J.
- **Studio verification:** Rojo sync needed — needs manual Studio playtest. Test: (1) sprint forward-left (A+W) with mouse lock ON → body rotates toward MoveDirection, RunForward plays; (2) sprint right (D) → RunForward plays, body faces right; (3) sprint backward → RunForward plays, body faces backward; (4) no RunForwardLeft or RunForwardRight ever selected; (5) no camera jerk; (6) body rotation feels natural at lerp alpha 0.18.

**Updated (2026-05-25 — Stage 3G: Smooth sprint body-facing toward MoveDirection):**
- **Stage 3E camera-jerk side-effect addressed:** Disabling Stage 3E (`SPRINT_USE_NATURAL_AUTOROTATE_WHILE_MOUSE_LOCKED = false`) removed character body-facing during sprint, leaving the character always facing camera yaw. Stage 3G restores directional body-facing without the camera jerk.
- **New `SPRINT_SMOOTH_BODY_FACING_ENABLED = true` constant:** Master switch for Stage 3G. When true, a new block in `applyCharacterFacing()` (after the Stage 3E exit block, before the camera-yaw write) calls `getCameraRelativeMoveDirection()` → `faceCharacterTowardsDirection()` during sprint + mouse lock, then returns early. `AutoRotate` remains `false` throughout — no Roblox physics rotation involved, so no camera jerk.
- **`SPRINT_DIRECTIONAL_BODY_FACING_SMOOTHING_ENABLED` changed `false → true`:** Stage 3D lerp smoothing now active. `faceCharacterTowardsDirection()` blends toward `MoveDirection` over multiple frames rather than snapping immediately.
- **`SPRINT_DIRECTIONAL_BODY_FACING_LERP_ALPHA` changed `1 → 0.18`:** Per-Heartbeat lerp alpha. At 0.18 (~60Hz), the character tracks direction changes within 3–4 frames, fast enough to feel responsive without the per-frame snap that the previous discrete-snap path (Stage 3D) caused.
- **Stage 3D helpers reactivated:** `getCameraRelativeMoveDirection()` and `faceCharacterTowardsDirection()` (both defined at Stage 3D) are now called by the Stage 3G block. They were dead code from Stage 3E until now.
- **Stage 3D discrete-snap path no longer used:** The Stage 3D `applyCharacterFacing()` sprint block (gated by `SPRINT_DIRECTIONAL_BODY_FACING_ENABLED`) is dead code — the Stage 3G block runs before it and returns early when conditions are met.
- **No Studio MCP verification yet.** Needs manual playtest.

- **Stage 3G new risk — lerp alpha tuning:** `SPRINT_DIRECTIONAL_BODY_FACING_LERP_ALPHA = 0.18` was chosen analytically (3–4 frame response at 60Hz). In live Studio play, if the body rotation feels too sluggish (character lags noticeably behind movement direction), increase toward 0.3–0.4. If it still feels snappy or produces micro-jitter on input noise, decrease toward 0.1. Tune only the constant, no code changes needed.
- **Stage 3G new risk — `getCameraRelativeMoveDirection()` magnitude threshold:** The minimum threshold `SPRINT_DIRECTIONAL_BODY_FACING_MIN_MOVE_MAGNITUDE = 0.1` governs when the Stage 3G block falls through to camera-yaw. If the player taps a direction key very briefly, movement magnitude may drop below 0.1 mid-sprint, causing a single-frame reversion to camera-yaw facing. Increase the threshold if this produces visible snapping; decrease if diagonal movement is not recognized.
- **Stage 3G new risk — body facing lags behind camera when sprint input is released:** `getCameraRelativeMoveDirection()` reads `movementState.moveVector` (Humanoid.MoveDirection). When the player releases all movement keys while sprinting, the vector goes near-zero — the Stage 3G block falls through to camera-yaw facing. The body snaps back toward camera yaw on the next Heartbeat. This may be visually noticeable at low lerp alpha. If the snap is jarring, add a "last sprint direction" cache and lerp toward it for a few frames after movement ends before switching to camera-yaw.
- **Stage 3G new risk — MCP Studio verification not yet performed:** Needs manual playtest: (1) mouse lock ON + sprint forward → body faces forward (same as camera yaw — no visible change); (2) sprint left (A key, mouse facing forward) → body rotates left over 3–4 frames, character body visually faces movement direction; (3) sprint forward-left diagonal → body tracks diagonal smoothly, RunForwardLeft animation plays; (4) release sprint (walk while mouse-locked) → body snaps back to camera yaw immediately (correctly falls through to camera-yaw write); (5) no camera jerk, no side-to-side camera oscillation; (6) tactical sprint, crouching, sprint-stop, landing lock → all still use camera-yaw facing (Stage 3G block guards exclude these); (7) AutoRotate stays false throughout; (8) no camera.CFrame, CameraOffset, FOV, HipHeight, JumpPower, or server changes.

**Updated (2026-05-25 — Stage 3H: Sprint camera offset override):**
- `updateSprintCameraOffset()` helper added. Called from Heartbeat after `updateSprintFov()`.
- When `SPRINT_DISABLES_CAMERA_OFFSET = true` and mouse lock is active: `Humanoid.CameraOffset = Vector3.zero` while `movementState.isSprinting or isTacticalSprinting`. Restores `CUSTOM_MOUSE_LOCK_CAMERA_OFFSET` on the next Heartbeat when sprint ends.
- Guarded by `~=` — no redundant per-frame writes.
- Rojo sync was pending at commit time — needs manual Studio playtest.

- **Stage 3H new risk — `applyCustomMouseLockCamera()` sets offset to 1.75 on toggle while sprinting:** If the player enables mouse lock while already sprinting, `applyCustomMouseLockCamera()` writes `CUSTOM_MOUSE_LOCK_CAMERA_OFFSET` (1.75, 0, 0) immediately. The Heartbeat `updateSprintCameraOffset()` corrects it to zero within one frame (~16ms). The 1-frame flash of the shoulder offset may be barely perceptible on a slow machine. If visible, add an explicit `updateSprintCameraOffset()` call at the end of `applyCustomMouseLockCamera()`.
- **Stage 3H new risk — offset restored on sprint end, not sprint-stop start:** `isSprintStopPlaying` is not a guard in `updateSprintCameraOffset()` — the function uses only `movementState.isSprinting or isTacticalSprinting`. When `isSprinting` becomes false at sprint end (which triggers sprint-stop), `updateSprintCameraOffset()` immediately restores the shoulder offset even while the sprint-stop animation is still playing. This is intentional (the player is no longer sprinting), but if the shoulder offset pop during sprint-stop is visually jarring, add `and not isSprintStopPlaying` to the wantZero condition.
- **Stage 3I new risk — diagonal body rotation tuning:** `SPRINT_DIAGONAL_BODY_ROTATION_DEGREES = 20` was set analytically (animation bakes ≈45° lean; total target = 45°; body rotation = 0–20°). In Studio, if the diagonal sprint still looks too close to backward, decrease the constant toward 10 or 0. If it looks too close to facing straight forward, increase toward 30–45. No code changes needed — constant is the sole tuning knob. Range: 0 (body faces camera-forward, animation provides all lean) to 45 (full MoveDirection rotation — same as non-diagonal Stage 3G behavior).
- **Stage 3I new risk — sign correctness for left/right rotation:** The 2D rotation formula uses `sign = -1` for ForwardLeft and `+1` for ForwardRight. This was derived from the standard right-hand Y-up 2D rotation matrix. If the rotation appears inverted in Studio (ForwardLeft rotates right, ForwardRight rotates left), negate `SPRINT_DIAGONAL_BODY_ROTATION_DEGREES` (or swap the sign values) — no other code changes needed.
- **Stage 3I new risk — lerp still applies:** `faceCharacterTowardsDirection()` with smoothing enabled (`SPRINT_DIRECTIONAL_BODY_FACING_SMOOTHING_ENABLED = true`) lerps toward the fixed-angle target over multiple frames (alpha = 0.18). The body will smoothly interpolate toward the 20° target, not snap to it. If the lerp feels too slow for diagonal sprints specifically, raise `SPRINT_DIRECTIONAL_BODY_FACING_LERP_ALPHA`.
- **Stage 3H new risk — Studio verification pending:** Rojo serve was not confirmed running at commit time. Needs manual playtest: (1) sprint with mouse lock ON → camera centers behind character (no shoulder offset); (2) release sprint → shoulder offset restores within one frame; (3) walk with mouse lock ON → shoulder offset present; (4) toggle mouse lock off → offset restores to zero (restoreNormalThirdPersonCamera, unchanged); (5) tactical sprint → camera also centers; (6) crouch while sprinting → isSprinting = false → offset restores before crouch animation starts; (7) no camera.CFrame, FOV, HipHeight, JumpPower, or server changes.

**Updated (2026-05-26 — Stage 3K: Tactical sprint mouse sensitivity override):**
- `applyTacticalSprintSensitivity(active: boolean)` private helper added. On `active=true`: caches `UserInputService.MouseDeltaSensitivity` into `tacticalSprintOriginalSensitivity`, writes `cached × TACTICAL_SPRINT_SENSITIVITY_MULTIPLIER`. On `active=false`: restores from cache and nils it. Idempotent in both directions.
- `tacticalSprintOriginalSensitivity: number?` module-level state variable added. Nil when no override is active.
- Two new Constants: `TACTICAL_SPRINT_SENSITIVITY_ENABLED = true`, `TACTICAL_SPRINT_SENSITIVITY_MULTIPLIER = 0.5`.
- `applyTacticalSprintSensitivity(true)` called at the one tactical sprint entry site (InputBegan double-tap path, after `isTacticalSprinting = true`).
- `applyTacticalSprintSensitivity(false)` called at all six exit sites: `stopTacticalSprint()`, `playSprintStopWithLock()`, `startLandingMovementLock()` (landing suppresses sprint flags), `loadMovementAnimations()` respawn block, phase exit handler, `destroy()`.
- MCP unavailable — Studio verification was not performed. Needs manual playtest.

- **Stage 3K new risk — sensitivity stuck if a new exit path is added:** All six current `isTacticalSprinting = false` sites have the restore call. If a future stage adds a seventh exit path (e.g. a stun, knockdown, or new phase-transition flow) and forgets to call `applyTacticalSprintSensitivity(false)`, sensitivity will remain at the reduced value until the next restore path fires. The idempotent nil-check makes erroneous extra calls safe, but missing calls are not. Review this list when any new `isTacticalSprinting = false` assignment is added.
- **Stage 3K new risk — sensitivity not zeroed at module load:** `tacticalSprintOriginalSensitivity` is nil at module load (correct). `UserInputService.MouseDeltaSensitivity` is not written until tactical sprint activates. If another system writes `MouseDeltaSensitivity` between module load and first tactical sprint entry, that value becomes the "original" that gets cached and restored — which is the correct behavior. No issue unless the other system also reads `MouseDeltaSensitivity` and expects it to be unchanged while tactical sprint is active.

...
- Tarkov-style hipfire (2026-09-09): `GunController` fires the hipfire raycast from the viewmodel `MuzzleAttachment` along its LookVector (`ViewModelController.GetMuzzleWorldCFrame()`) — the viewmodel is rotated toward the free-aim reticle (`FREE_AIM_VIEWMODEL_TRACK_FACTOR`), so bullets leave where the gun points, not a camera ray through a moving reticle. ADS / no-muzzle fall back to the camera path. Gated by `Constants.FREE_AIM_FIRE_FROM_MUZZLE`. Wires the old DEBT-063. Per-weapon feel via `Constants.FREE_AIM_PROFILES` (`DEFAULT` + `AKS74` + `AR15`), resolved in `FreeAimController:SetWeapon(name)` and `ViewModelController` on equip. The OS cursor is locked to centre + hidden while armed in ACTIVE (`Constants.FIRST_PERSON_AIM`, `GunController`, restores the pre-lock `MouseBehavior` so `MovementController`'s LeftControl lock is preserved). The fixed centre crosshair is hidden during hipfire (`CrosshairUI:SetHipfireActive`); the floating gun-direction reticle is the hipfire crosshair. Client-only: no server, no remotes, no ammo/damage/hit-validation change (server still re-raycasts the client origin+direction). Not runtime-verified — see TECHNICAL_DEBT (yaw sign, cursor lock, per-gun feel need an in-game test).
